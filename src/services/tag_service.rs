...
impl TagService {
    pub async fn get_tag(&self, user_id: &str, tag_id: &str) -> Result<TagDTO, ApiErrors> {
        let find_result = self.repository.find_by_id(user_id, tag_id).await;
    ...
    pub async fn create_tag(&self, user_id: &str, tag: NewTagDTO) -> Result<TagDTO, ApiErrors> {
        create_merged(
            tag,
            async move |created_tag_id| self.get_tag(user_id, &created_tag_id).await,
            async move |tag_to_create| {
                let exists_result = self
                    .repository
                    .exists_with_unique(user_id, &tag_to_create)
                    .await;
                handle_already_exists_result::<TagDTO>(exists_result)?;

                let create_result = self.repository.create(user_id, &tag_to_create).await;
    ...
    pub async fn update_tag(
        ...
        update_merged(
            tag,
            async move || self.get_tag(user_id, tag_id).await,
    ...
    pub async fn delete_tag(&self, user_id: &str, tag_id: &str) -> Result<(), ApiErrors> {
        self.exists_tag(user_id, tag_id).await?;

        let delete_result = self.repository.delete_by_id(user_id, tag_id).await;
        handle_action_result::<TagDTO>(delete_result)
    }

    pub async fn exists_tag(&self, user_id: &str, tag_id: &str) -> Result<(), ApiErrors> {
        let exists_result = self.repository.exists_by_id(user_id, tag_id).await;
