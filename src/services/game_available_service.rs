...
pub async fn get_game_locations(
    ...
    let find_result =
        game_available_repository::find_all_locations_with_game(pool, user_id, game_id).await;
    handle_get_list_result(find_result)
}
...
pub async fn create_game_available(
    ...
    let create_result =
        game_available_repository::create(pool, user_id, game_id, location_id, available_date)
            .await;
...
pub async fn delete_game_available(
    ...
    exists_game_available(pool, user_id, game_id, location_id).await?;

    let delete_result =
        game_available_repository::delete_by_id(pool, user_id, game_id, location_id).await;
...
pub async fn exists_no_game_available(
    ...
    let exists_result =
        game_available_repository::exists_locations_with_game(pool, user_id, game_id).await;
